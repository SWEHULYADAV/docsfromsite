...
def scrape_link(driver, link):
    ...
    try:
        ...
        if entire_year_link:
            ...
            for link in valid_links:
                ...
                if pdf_button:
                    ...
                    response = requests.get(pdf_url, stream=True)
                    if response.status_code == 200:
                        with open(os.path.join("pdfs_folder", "document.pdf"), "wb") as f:
                            for chunk in response.iter_content(1024):
                                f.write(chunk)
        ...
        print(f"No 'Entire Year' link found for URL: {link}")
...
def scrape_indian_kanoon(driver):
    ...
    # Create a queue to store the links
    link_queue = Queue()
    ...
    # Create a thread pool to scrape the links in parallel
    num_threads = 5  # Adjust the number of threads as needed
    threads = []
    for i in range(num_threads):
        thread = threading.Thread(target=scrape_link, args=(driver, link_queue.get()))
